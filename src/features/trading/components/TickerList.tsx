...
const TickerItem: React.FC<TickerItemProps> = React.memo(
  ({ pair, ticker, isSelected, isFavorite, onSelect, onToggleFavorite }) => {
    ...
    return (
      <div
        ...
        className={`group relative flex gap-3 items-center w-full p-2 text-xs transition-colors cursor-pointer ${
          isSelected ? "bg-white/10" : "hover:bg-[var(--color-binance-hover)]"
        }`}
      >
        <div className="w-4 max-w-4">
          <FavoriteStar
        ...
        <div className="text-left font-medium text-foreground truncate overflow-hidden whitespace-nowrap min-w-[4.8rem] 2lg:min-w-[7.5rem]">
          {pair.replace(/USDT|BTC|ETH|BNB$/, "")}
          <span className="text-muted-foreground">
            /{pair.match(/USDT|BTC|ETH|BNB$/)?.[0]}
          </span>
        </div>

        <div
          className={`text-right min-w-11 font-mono ${
        ...
        >
          {ticker ? (
            ...
            <SkeletonLoader className="w-10 h-3 inline-block" />
          )}
        </div>
        ...
        <div className="absolute z-10 hidden group-hover:flex flex-col px-2 py-1 rounded bg-black text-white text-[10px] whitespace-nowrap left-1/2 -translate-x-1/2 bottom-full mb-1">
          <div>
          ...
          <div>
            ...
            {ticker ? parseFloat(ticker.c).toFixed(4) : "-"}
          </div>
          <div>
...
const TickerList: React.FC<{ activeSymbol: string }> = ({ activeSymbol }) => {
  ...
  const [favorites, setFavorites] = useState<Set<string>>(() => {
    ...
    const saved = localStorage.getItem("tickerFavorites");
  ...
  useEffect(() => {
    if (listContainerRef.current) {
      const observer = new ResizeObserver(
        (entries) => entries[0] && setListHeight(entries[0].contentRect.height)
      );
  ...
  const toggleFavorite = useCallback((pair: string) => {
    setFavorites((prev) => {
      ...
      if (newFavs.has(pair)) newFavs.delete(pair);
      else newFavs.add(pair);
      return newFavs;
    });
  }, []);
  ...
  useEffect(() => {
    const fetchSymbols = async () => {
      ...
      try {
        ...
        setAllSymbols(await response.json());
      } catch (err) {
        setErrorSymbols(err instanceof Error ? err.message : "Unknown error");
      } finally {
  ...
  const handlePairSelect = (pair: string) => {
    if (pair !== activeSymbol) router.push(`/trade/${pair}`);
  };

  const filteredPairs = useMemo(() => {
    let pairsToShow: string[] = [];
    if (activeTab === "⭐️") {
      pairsToShow = Array.from(favorites).sort();
    } else {
      pairsToShow = allSymbols.filter((p) => p.endsWith(activeTab));
    }
    if (searchTerm) {
      return pairsToShow.filter((p) =>
  ...
  const renderContent = () => {
    ...
    if (filteredPairs.length === 0) {
      if (activeTab === "⭐️")
        return (
          <div className="p-4 text-center text-muted-foreground">
            Click the star icon to add a market to your favorites.
          </div>
        );
      return (
        <div className="p-4 text-center text-muted-foreground">
    ...
    return (
      <div ref={listContainerRef} className="flex-1 w-full h-full min-h-0">
        {listHeight > 0 && (
          <List
            height={listHeight}
  ...
  return (
    <div className="h-full flex flex-col text-foreground bg-muted p-1">
      <div className="flex-shrink-0 flex items-center border-b border-border">
        {quoteAssets.map((asset) => (
          <QuoteAssetTab
            ...
            onClick={() => setActiveTab(asset.toString())}
          />
        ))}
      </div>
      <div className="p-2">
      ...
      <div className="grid grid-cols-4 gap-3 px-2 pb-1 text-xs text-muted-foreground">
        <div className="w-4 max-w-4" />
        {/* <div /> */}
